...
class PlayerView: UIView {
    ...
    func seek(to percent: Double) {
        self.player?.seek(
            // preferredTimescale: seconds를 얼마나 쪼갤지
            to: CMTime(seconds: percent * self.totalPlayTime, preferredTimescale: 1)
        )
    }

    func forward(to seconds: Double = 10) {
        guard let currentItem = self.player?.currentItem?.currentTime().seconds else {
            return
        }
        self.player?.seek(
            to: CMTime(seconds: currentItem + seconds, preferredTimescale: 1)
        )
    }
